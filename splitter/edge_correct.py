...
def hough_line_detect(img, raw_img):
    ...
    cv2.imwrite('../data/test_10/1/mid.jpeg', img_copy)
    # img_copy = cv2.dilate(img_copy, np.uint8([[0, 1, 0], [0, 1, 0], [0, 1, 0]]), iterations=2)

    # 概率直线检测
    lines = cv2.HoughLinesP(img_copy, 1, np.pi/180, int(highy * 0.1), minLineLength=highy * 0.2, maxLineGap=3)
    if isinstance(lines, np.ndarray):
        lines = np.reshape(lines, [-1, 4])
        for line in lines:
            if abs(line[0] - line[2]) > abs(line[1] - line[3]):
                continue
            if abs(line[0] - line[2]) < lenx / 8 and abs(line[0] - lenx/2) < lenx / 6:
...
def hough_line_detect_with_header(img, raw_img):
    """
    输入图片，进行直线识别
    :param img:图片矩阵
    :return:
    ...
    # 左右膨胀，迭代三次
    img_copy = cv2.dilate(img_copy, np.uint8([[0, 0, 0], [1, 1, 1], [0, 0, 0]]), iterations=3)
    img_copy = cv2.dilate(img_copy, np.uint8([[0, 1, 0], [0, 1, 0], [0, 1, 0]]), iterations=1)

    # img_copy = cv2.dilate(img_copy, np.uint8([[0, 1, 0], [0, 1, 0], [0, 1, 0]]), iterations=2)

    # 概率直线检测
    lines = cv2.HoughLinesP(img_copy, 1, np.pi/180, int(highy * 0.1), minLineLength=highy * 0.3, maxLineGap=3)
    if isinstance(lines, np.ndarray):
        lines = np.reshape(lines, [-1, 4])
        for line in lines:
