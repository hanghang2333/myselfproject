...
dataroot = ''#数据所在目录
catename = ''#该数据类别对应真实名称文件，可能会生成如果不存在的话。该文件生成方式:如果不存在，则根据本次读取的label2label文件生成。所以，在label2label修改后该文件也应该修改。
label2label_path = ''#类别到类别对应文件

def init(t='X'):
    global namepath,dataroot,catename,label2label_path
    if t =='X':
        dataroot = '/home/lihang/ocr/master/data/data_0/cate_data/1025/'
        catename = 'catenamelabel'
        label2label_path = 'label2label.list'
    elif t == 'N':
        dataroot = '/home/lihang/ocr/master/Ndata/data_0/cate_data/1101data_add/'
        catename = 'Ncatenamelabel'
        label2label_path = 'Nlabel2label.list'
...
def reshape(im,height,width):
    ...
    b = im
    flag = 0
    if b.size[1]>height or b.size[0]>width:
        flag = 1
        b = b.resize((width,height),Image.BILINEAR)
    b = np.reshape(b,[b.size[1],b.size[0],1])
    return b

def normal(data,height1,width1):
    '''
    归一化
    '''
    height = data.shape[0]
    ...
    data = (data -mean)/std
    #pad
    hpad = max(0,height1-height)
    wpad = max(0,width1-width)
    data = np.pad(data,((0,hpad),(0,wpad),(0,0)),mode='constant')
    data = data[0:height1,0:width1,:]
    return data
...
def get(image_height,image_width,image_channel):
    ...
    Y = np.zeros((image_num,1),np.uint16)
    all_path = []
...
#X, Y = get(sess, image_height, image_width, image_channel)
