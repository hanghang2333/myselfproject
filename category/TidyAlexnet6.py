...
def get_center_loss(features, labels, alpha, num_classes):
    ...
    diff = tf.multiply(centers_weight,tf.multiply(centers_weight,centers_batch-features))#centers_batch - features
...
class alexNet(object):
    ...
    def __init__(self, image_height, image_width, image_channel, keep_prob, classNum):
        ...
        with tf.variable_scope('loss_scope'):
            ...
            self.loss = tf.losses.sparse_softmax_cross_entropy(labels=self.y, logits=self.score)+0.01*self.centerloss+self.regularization_loss
    ...
    def buildCNN3(self):
        ...
        roiout = roi_pooling(hidden3_1,rois_const,pool_height=3,pool_width=5)
        roiout = tf.transpose(roiout,[0,2,3,1])
        flatten = tf.reshape(roiout, [-1,15*256])
        
        hidden3_1 = tf.layers.max_pooling2d(hidden3_1, pool_size=[2,2], strides=(2,2), padding='valid')
    ...
    def train(self, sess, X_train, Y_train, X_test, Y_test, split,num_epochs, num_count,test_path,logs):
        ...
        batch_size = int(len(X_train)/split)
