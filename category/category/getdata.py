...
def get_path_label_2(rootpath):
    ...
    for label in label1:#对每一个label
        try:
            filelist = os.listdir(rootpath+label.encode('utf8'))
            for everyfile in filelist:#对每一个label目录下的文件
                pathlabel[rootpath+label.encode('utf8')+'/' + everyfile] = l2lidx_dict[l2l_dict[label]]
        except Exception:
            print(rootpath+label.encode('utf8'))#有些在大类文件里的小类实际上数据里并没有对应的文件夹
