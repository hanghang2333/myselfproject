...
class alexNet(object):
    ...
    def __init__(self, image_height, image_width, image_channel, keep_prob, classNum):
        ...
        '''
        # 数据占位
        self.X = tf.placeholder(tf.float32,[None,30,150,1],name='inputX')
        ...
        with tf.control_dependencies(self.extra_update_ops):
            self.train_op = tf.train.MomentumOptimizer(0.0001, 0.9).minimize(self.loss)
        # 准确度定义
    ...
    def buildCNN3(self):
        ...
        with tf.variable_scope('hidden1',regularizer=self.regularizer,initializer=self.init):
            conv = tf.layers.conv2d(self.X,filters=16,kernel_size=[5,5],strides=(1,3),padding='valid')
        ...
        #roi pooling
        #import math
        ##print(hidden3_1.shape[1].value)
        #height = hidden3_1.shape[1]
        #width = hidden3_1.shape[2]
        #hidden3_1 = tf.layers.max_pooling2d(hidden3_1, pool_size=[height/3,width/3], strides=(height/3,width/3), padding='valid')
        #hidden3_1 = tf.keras.layers.GlobalAveragePooling2D(hidden3_1)
        #flatten = tf.reshape(hidden3_1,[-1,256])
        #use api
        rois_value = [[0,0,0,5,10] for i in range(128)]
        rois_value = np.asarray(rois_value, dtype='int32')
        rois_const = tf.constant(rois_value, tf.int32)
        hidden3_1 = roi_pooling(hidden3_1,rois_const,pool_height=3,pool_width=5)
        hidden3_1 = tf.transpose(hidden3_1,[0,2,3,1])
        flatten = tf.reshape(hidden3_1, [-1,15*256])
    ...
    def train(self, sess, X_train, Y_train, X_test, Y_test, split,num_epochs, num_count,test_path,logs):
        ...
        for e in range(num_epochs):
            ...
            for x_batch, y_batch in datagen.flow(X_train, Y_train, batch_size=batch_size, save_to_dir=None):
                if batches == max(split,50):
                    ...
                    for i in range(len(X_test)):
                        a = []
                        loss = []
                        now_X_test = X_test[i:i+1,:,:,:]
                        now_Y_test = Y_test[i:i+1,:,:,:]
                        ...
                        loss.append(lossval)
                        a.append(accuval)
                    print("Test accuracy:", sum(accuval)*1.0/len(X_test))
                    print("Test loss:",sum(lossval)*1.0/len(X_test))
            ...
            for i in range(len(X_train)):
                ...
                now_X_train = np.array([X_train[i:i+1]])
                #print(now_X_test.shape)
                now_Y_train = Y_train[i:i+1]
                #print(now_Y_test.shape)
                summary2, accuval, scoreval, lossval = sess.run([self.merged, self.accuracy_test, self.score,self.loss],
                                                     feed_dict={self.X: now_X_train, self.keep_prob:0.5, self.y:now_Y_train,self.is_training:True})
                loss1.append(lossval)
                a1.append(accuval)
            print("Train accuracy:", sum(a1)*1.0/len(X_train),a1[0:5])
            print("Train loss:",sum(loss1)*1.0/len(X_train))

            a = []
            loss = []
            for i in range(len(X_test)):
                #print(X_test[i].shape)
                now_X_test = np.array([X_test[i]])
                #print(now_X_test.shape)
                now_Y_test = Y_test[i:i+1]
                #print(now_Y_test.shape)
                summary2, accuval, scoreval, lossval = sess.run([self.merged, self.accuracy_test, self.score,self.loss],
                                                     feed_dict={self.X: now_X_test, self.keep_prob:1, self.y:now_Y_test,self.is_training:False})
                loss.append(lossval)
                a.append(accuval)
            print("Test accuracy:", sum(a)*1.0/len(X_test),a[0:5])
            print("Test loss:",sum(loss)*1.0/len(X_test))
            '''
            for i in range(1):
            ...
            for i in range(1):
                a1 = []
                loss1 = []
                for bx,by in yieldTest.get_next_batch(batch_size):
                    summary2, accuval, scoreval, lossval = sess.run([self.merged, self.accuracy_test, self.score,self.loss],
                                                 feed_dict={self.X: bx, self.keep_prob:1, self.y:by,self.is_training:False})
                    a1.append(accuval)
                    loss1.append(lossval)
                print("Test accuracy:",sum(a1)/(int(len(X_test)/batch_size)))
                print("Test loss:",sum(loss1)/(int(len(X_test)/batch_size)))
        return 0
